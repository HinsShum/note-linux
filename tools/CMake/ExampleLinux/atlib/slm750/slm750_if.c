...
{
    ryat_cmd_t cmd = {__ms_to_tick(1000), "AT+CELLINFO\r\n", RYAT_RET_CELL_INFO | RYAT_RET_ERROR};
    ryat_rx_line_t *prx = NULL;
    ryat_utils_args_t args = {0};

    cmd.cmdlen = strlen(cmd.cmd);
    if(RYAT_RET_CELL_INFO != ryat_set(gp_at, &cmd)) {
        return RYAT_E_ERROR;
    }
    // get cell id
    if(NULL == (prx = ryat_get_line_with_prefix(gp_at, " CELL_ID"))) {
        return RYAT_E_ERROR;
    }
    if(RYAT_E_OK != ryat_utils_get_args(prx->buf, prx->len, &args)) {
        return RYAT_E_ERROR;
    }
    pcell->cellid = strtoul(args.pnext, NULL, 16);
    // get lac
    if(NULL == (prx = ryat_get_line_with_prefix(gp_at, " LAC_ID"))) {
        return RYAT_E_ERROR;
    }
    if(RYAT_E_OK != ryat_utils_get_args(prx->buf, prx->len, &args)) {
        return RYAT_E_ERROR;
    }
    pcell->lac = strtoul(args.pnext, NULL, 16);
