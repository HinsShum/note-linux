...
static bool bsp_init(void)
{
    ...
    if(RS232_OpenComport(com.comport, com.baudrate, mode)) {
        retval = false;
        printf("Open /dev/ttyS%d failed\n", com.comport);
    }
...
static void bsp_deinit(void)
{
    RS232_CloseComport(com.comport);
}

static uint16_t bsp_write(uint8_t *pbuf, uint16_t length)
{
    RS232_SendBuf(com.comport, (unsigned char *)pbuf, length);
